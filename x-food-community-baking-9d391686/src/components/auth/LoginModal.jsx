...
export default function LoginModal({ isOpen, onOpenChange }) {
  const handleLogin = async () => {
    // Construct the full URL for redirection after login
    const callbackUrl = window.location.origin + createPageUrl("AddBake");
      ...
      <DialogContent className="sm:max-w-md bg-white/90 backdrop-blur-md border-purple-200 shadow-2xl rounded-2xl">
        ...
        <div className="mt-6">
          <Button
            onClick={handleLogin}
            size="lg"
            className="w-full bg-gradient-to-r from-orange-500 to-pink-500 text-white shadow-lg hover:shadow-xl transition-all text-lg font-semibold py-6 rounded-xl"
          >
