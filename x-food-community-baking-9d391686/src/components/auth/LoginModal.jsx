...
export default function LoginModal({ isOpen, onOpenChange }) {
  ...
  useEffect(() => {
    ...
    if (window.AppleID && AUTH_CONFIG.APPLE.CLIENT_ID !== 'YOUR_APPLE_CLIENT_ID') {
      try {
        window.AppleID.auth.init({
          clientId: AUTH_CONFIG.APPLE.CLIENT_ID,
          scope: 'name email',
          redirectURI: AUTH_CONFIG.APPLE.REDIRECT_URI,
          state: 'xfood-auth',
          usePopup: true
        });
        console.log('Apple Sign-In initialized with client ID:', AUTH_CONFIG.APPLE.CLIENT_ID);
      } catch (error) {
        console.error('Failed to initialize Apple Sign-In:', error);
        setError('Apple Sign-In initialization failed. Please refresh the page.');
      }
    } else {
      console.log('Apple Sign-In not available or client ID not configured');
    }
  }, []);
  ...
  const handleAppleLogin = async () => {
    if (!window.AppleID) {
      setError('Apple Sign-In not loaded. Please refresh the page and try again.');
      return;
    }

    if (AUTH_CONFIG.APPLE.CLIENT_ID === 'YOUR_APPLE_CLIENT_ID') {
      setError('Apple Sign-In needs to be configured. Please contact support.');
      return;
    }
    ...
    try {
      console.log('Attempting Apple login with client ID:', AUTH_CONFIG.APPLE.CLIENT_ID);
      ...
      if (data.authorization && data.authorization.id_token) {
        const result = await login(data.authorization.id_token, 'apple');
        if (result.success) {
          onOpenChange(false);
          // Navigate to profile after successful login
          window.location.href = '/Profile';
      ...
      if (error.error === 'popup_closed_by_user') {
        ...
        setError(`Apple login failed: ${error.message || 'Unknown error'}`);
      }
    } finally {
      setIsLoading(false);
    }
      ...
      <DialogContent className="sm:max-w-md bg-white/90 backdrop-blur-md border-purple-200 shadow-2xl rounded-2xl">
        ...
        <div className="mt-6 space-y-4">
          ...
          <Button
            ...
            size="lg"
            variant="outline"
            className="w-full border-2 border-gray-800 bg-black text-white hover:bg-gray-800 shadow-lg hover:shadow-xl transition-all text-lg font-semibold py-6 rounded-xl disabled:opacity-50"
          >
